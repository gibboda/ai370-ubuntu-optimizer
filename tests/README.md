...
- Agent contract compatibility tests:
  ...
  authoritative)
- Agent distribution contract tests: `test_agent_distribution_contract.py`
  validates the portable-vs-local package boundary, immutable source lock,
  PR-only synchronization, fail-and-review drift handling, and the
  prohibition on overwriting repository-local policy
...
- Independent review is local Grok Build advisory review. Assigned advice
  must be recorded as a COMMENT-only pull-request comment or COMMENT
  review. Antigravity CLI (`agy`) remains specialist-only and is not an
  independent-review fallback if Grok is unavailable.
