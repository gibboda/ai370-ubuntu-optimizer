...
- `tier2-pytorch-rocm.json`, `tier2-llama-cpp.json`, `tier2-ollama.json`, `tier2-open-webui.json`, `tier2-runtime-benchmark.json`, `llm-validation.json`, and `tier2-validation.json` record Milestone 2 / Stage 2 runtime installation, local model visibility, and gate status.
  Tier 2 PyTorch installation removes stale PyTorch package cache entries, installs `torch`, `torchvision`, and `torchaudio` together from the selected PyTorch index, and automatically uses the configured nightly/pre-release index for Python runtimes that need newer wheels, such as Python 3.14+.
