#!/usr/bin/env python3
"""S2-M4: validate and atomically publish NPU runtime visibility report."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="S2-M4 publish s2-m4-npu-runtime-validation.json from visibility checks"
    )
    parser.add_argument("--checks", required=True, type=Path, help="NPU visibility checks JSON")
    parser.add_argument(
        "--profile",
        type=Path,
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="Canonical s2-m4-npu-runtime-validation.json destination",
    )
    ...
    report = capability_ladder.build_s2_m4_visibility_report(hardware, checks, consumed)
    system_profile.atomic_write_document(
        args.output,
        report,
        capability_ladder.S2_M4_SCHEMA,
        "S2-M4",
    )
    print(f"[INFO] S2-M4 NPU runtime visibility written to {args.output}")
