...
main() {
  local title="${1:-}"
  local allowed_types allowed_scopes milestone_scope pattern
  ...
  allowed_scopes='audit|baseline|amd|ai-stack|rocm|npu|acceleration|comfyui|config|architecture|agents|governance|mcp|contract|workflows|vscode|workspace|settings|release|changelog|deps|stage|stage1|stage2|stage3|stage4|stage5|tier|tier1|tier2'
  ...
  if [[ ! "$title" =~ $pattern ]]; then
    ...
    echo "[ERROR] Allowed scopes: audit, baseline, amd, ai-stack, rocm, npu, acceleration, comfyui, config, architecture, agents, governance, mcp, contract, workflows, vscode, workspace, settings, release, changelog, deps, stage, stage1, stage2, stage3, stage4, stage5, tier (deprecated), tier1 (deprecated), tier2 (deprecated), or canonical milestone scopes like s5-m6" >&2
