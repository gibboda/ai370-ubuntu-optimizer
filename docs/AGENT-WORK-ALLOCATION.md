...
Grok Build is the primary independent AI challenge/review provider and may also provide specialist advice. A CODEOWNER-assigned second look records Grok Build as `independent_review`. If Grok Build is unavailable, `agy` remains `independent_review` (independent-reviewer fallback). A CODEOWNER-assigned second look records Antigravity as `specialist_review`, and that second-look assignment is not `specialist_review` when `agy` is used as the independent-reviewer fallback. Assigned advice remains advisory and should be recorded as a COMMENT-only pull-request comment or COMMENT review.
