...
* Stage 1 is **implemented**, with an incremental rewrite toward a read-only hardware detector and system-profile builder now underway. `scripts/10-detect-hardware.sh` publishes the versioned `reports/latest/system-profile.json` defined by `configs/schemas/system-profile.schema.json`; existing Tier 1 artifacts and gates remain compatibility outputs during migration. The current canonical command surface remains: `scripts/10-detect-hardware.sh` (incl. NPU; `75` wrapper), `scripts/20-check-bios.sh` (BIOS+firmware; `25` wrapper), `scripts/30-validate-kernel.sh`, `scripts/40-platform-tuning.sh` (CPU+memory+storage plan; `40`/`50`/`60` wrappers), `scripts/70-validate-gpu-stack.sh`, optional `scripts/80-benchmark-local-ai.sh` (`--with-ai-smoke`), and `scripts/90-validate.sh` (`inventory` | `full` | `smoke`). Orchestrator: `stage1`, `stage1-inventory`, `stage1-validate` (`--inventory`); Package E flags `--strict`, `--apply-tuning`, `--with-ai-smoke`. Moving tuning, benchmarks, and offline handling out of Stage 1 remains follow-up work after Stage 2 consumes the new profile.
