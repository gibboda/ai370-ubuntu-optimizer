...
The current contract is schema version 4.
...
Version 4 changes the required interpretation of `independent_reviewer.providers`: the eligible set is exclusively `["grok_build"]`. It also adds `independent_reviewer.exclusive` and `invariants.grok_exclusive_independent_review`, and updates overlay `required_any` markers so Antigravity is not an independent reviewer. A schema 3 consumer that treated `antigravity_cli` as a Grok-unavailable independent-review provider cannot safely consume version 4. Removing that previously supported independent-review resource increments `architecture_contract_version` from `2` to `3`. Merge authority and advisory-review semantics stay unchanged; consumers must still require schema 4 or newer for the exclusive provider set.
...
For the version-3-to-version-4 transition:

1. Version-1 and version-2 role field values remain present.
2. `independent_reviewer.providers` changes from `["grok_build", "antigravity_cli"]` to `["grok_build"]`.
3. Consumers that enforce the eligible independent-review set must require schema version 4 or newer.
4. Overlay `required_any` markers for Grok and Antigravity change to exclusive-review language. Preserve path and role identity; do not require exact schema-3 marker strings.
